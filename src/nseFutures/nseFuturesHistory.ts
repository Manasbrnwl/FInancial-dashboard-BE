...
async function getNseFuturesHistory(date: string) {
  ...
  try {
    ...
    const response = await axios.get(
      `https://history.truedata.in/getbhavcopystatus?segment=FO&date=${date}&response=json`,
      {
        headers: {
    ...
    devError("Error fetching NSE Futures history:", error);
    prodError("Error fetching NSE Futures history");
    return false;
  }
}
