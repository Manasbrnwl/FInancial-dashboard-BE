...
// Main sync function - discovers new instruments/symbols from NSE Bhavcopy files
export async function syncHistoricalSymbols(startDate: string, endDate: string, dryRun = false): Promise<void> {
    console.log(`🚀 Starting Historical Symbols Sync (DRY_RUN = ${dryRun})`);
...
// CLI entry: npx ts-node src/scripts/fetchHistoricalSymbols.ts [--from YYYY-MM-DD] [--to YYYY-MM-DD] [--dry-run]
if (require.main === module) {
    const args = process.argv.slice(2);

    const eightDaysAgo = new Date();
    eightDaysAgo.setDate(eightDaysAgo.getDate() - 8);
    let from = eightDaysAgo.toISOString().split("T")[0];
    ...
    syncHistoricalSymbols(from, to, dryRun)
        .then(async () => {
